...
- ⏸ **列宽算法**：固定布局（`tblLayout=fixed`）优先，`autofit` 做简化版。
  纯水平方向，**不被基线穿刺阻塞**，是 Mac 上能接着做的下一块
- ⏸ 边框冲突解析（Word 的边框优先级规则）—— 属性都收齐了，规则本身还没写
...
12b. ~~**Phase 4 的 model 侧**：表格 / 行 / 格属性 + 级联 + 条件格式~~ ✅ 同样与基线无关。
    接着做的是 **`@uw/layout` 的列宽算法**（`tblGrid` + `tcW` + `gridSpan` → 每格的 x 与可用宽度），
    它是表格里唯一不需要 y 的部分，做完表格就跟段落一样「排到行盒门口」
