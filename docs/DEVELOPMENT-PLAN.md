...
- ⏸ **悬挂标点按半宽计入行宽** —— **这是 Phase 2 剩下的第一件事，量已经在真值里了，缺的是实现。**
  实测（gongwen-01 第 5 行）：悬挂出去的「，」左边缘落在版心线**内** 7.96pt、右边缘出界 8.05pt，
  也就是**墨留在版心内，只有空的那半边吐出去**；而 `contentWidth()` 现在把悬挂项整个不计入行宽。
  这一条是 L2 剩下 10 行对不上的直接原因（`layout/src/fixture.test.ts` 的 `MIN_L2_MATCH`）。
  它会同时改动两端对齐（拉伸量按行宽算）与「行不超出版心」那条断言，所以单独做一步，
  做完把 `MIN_L2_MATCH` 往上调
...
> 相邻标点挤压那条用 26 个样本定死并搬去了 `break-class.ts`。
...
13. **上 Windows 之后按这个顺序补**：~~① 基线穿刺（第 6 步，解锁行盒与分页）~~ ✅
    ...
    剩下的样本：临时挤压的上限、1/8 em 中西文间距（gongwen-01 测不了它 —— 那份文档中英之间
    本来就有空格，实测间隙为 0）、上下标字号、禁则集边界。
    另有**已量到、缺实现**的一条（悬挂标点按半宽计入行宽），它不是样本问题，
    列在 Phase 2 的待做项里，也是接着该做的第一件事
...
Windows 侧剩下的活是第 13 步的 ②–⑥，它们都不挡分页 —— 度量包影响的是断行点的精度
（L2/L4 断言），uncalibrated 的那几个常数同理。
