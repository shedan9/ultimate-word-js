...
13. **上 Windows 之后按这个顺序补**：~~① 基线穿刺（第 6 步，解锁行盒与分页）~~ ✅
    ...
    - 剩下没答的：**图片参不参与网格吸附**（这两份样本都关着网格）。
      一份「开行网格 + 一张比网格高的内嵌图」的样本就能钉死，公文都开着网格，
      优先级不低
...
2. ~~**图片**（`w:drawing`）~~ ✅ 四层都通了，几何也标定完了（第 12l 步）。
   还剩一问：**图片参不参与行网格吸附** —— 两份样本都是关着网格量的，而公文都开着网格
...
5. **表格的真实样本**：渲染器已经会画格线与底纹了，但格线冲突的三条规则、隔行带的序号
   ...
   拆行（第 12m 步）又给这份样本加了三问：接缝上画不画横线、单元格边距在两片上怎么分、
   `w:trHeight` 的富余落在哪一片 —— 一张「第二行塞十几段文字逼它跨页」的表一次全钉死
