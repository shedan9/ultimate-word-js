...
5. **表格剩下的一组样本**（几何、隔行带、格线冲突都做完了，见第 12p / 12q 步）：
   **拆行**的三问（接缝上画不画横线、单元格边距在两片上怎么分、`w:trHeight` 的富余
   落在哪一片）。一张「第二行塞十几段文字逼它跨页」的表就能全钉死，造样本的工具会造表，
   读线那一路真值（`pages[].rules[]`）第 12q 步也建好了 —— 接缝上画不画线正是读线的活
