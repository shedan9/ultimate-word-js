...
3. **标点挤压 / 溢出**
   ...
   - 塞不下时的**临时挤压有个上限，至今没标定** —— Word 接受过 9.30pt、拒绝过 13.75pt，
     这是 L2 剩下 7 行对不上的唯一原因，证据表在 `layout/src/uncalibrated.ts`
...
- ⏸ **临时挤压的上限** —— L2 剩下 7 行全从真值第 6 行连锁下来，那一行 Word 宁可换行
  也不肯再挤 14.16pt 的标点，而我们挤了。这**不是缺实现，是缺样本**：Word 在同一份文档里
  接受过 9.30pt、拒绝过 13.75pt，按标点算的模型还会自相矛盾（第 4 行一个标点交出 0.47 em
  被接受，第 6 行两个标点各交 0.44 em 被拒）。要造的 spike 写在 `uncalibrated.ts` 的
  `PUNCT_COMPRESS_RATIO`：挤压量从 0.1 em 走到 0.9 em × 标点个数 1/2/4
- ⏸ DOM 渲染器 v1：绝对定位 + SVG text 逐字 x —— 等分页给出页与 y
- **DoD**：单页纯文本公文，与 Word 截图叠加对比，字形位置误差 < 1pt；L2 逐行断行点与真值一致
  （当前 **11 / 18 行**，闸门 `MIN_L2_MATCH` 只许往上调）

> 未标定的常数（临时挤压的上限、自动间距的 1/8 em、上下标与小型大写的字号系数、内建禁则集）
> 集中在 `packages/layout/src/uncalibrated.ts`，每一条都写了「拿什么样本能钉死」。
> 它们影响的都是**宽度**，也就是断行点；行高与基线那两条已经分别用 13 / 30 个样本定死了，
> 相邻标点挤压（含「接缝上要有空白」）与悬挂的半宽两条也已实测，搬去了 `break-class.ts`。
...
- ⏸ 跨页：拆行、`cantSplit`、`tblHeader` 重复表头 —— 要分页（行盒已经有了）
...
13. **上 Windows 之后按这个顺序补**：~~① 基线穿刺（第 6 步，解锁行盒与分页）~~ ✅
    ...
    剩下的样本，**按价值排**：
    · **临时挤压的上限**（最值钱：它是 L2 剩下 7 行的唯一原因，样本设计写在
      `uncalibrated.ts` 的 `PUNCT_COMPRESS_RATIO`，一份 spike 就能钉死）
    · 1/8 em 中西文间距（gongwen-01 测不了它 —— 那份文档中英之间本来就有空格，
      实测间隙为 0；那个空格本身已经确认是东亚字体的 0.5 em）
    · 上下标字号、禁则集边界
    · 空格分桶的两处边界：`hint="default"` 时空格算谁的、`/` `-` 这类中性字符跟不跟着走
    （原先列在这里的「悬挂标点按半宽」已经做完，见第 12d 步）
...
现在它做完了，行盒有了基线，**下一个卡口变成分页**（Phase 3）：段落自己不知道它排在第几页的
哪个高度上，`LineLayout` 因此有 `baseline` 而没有 `y`。
Windows 侧剩下的活是第 13 步的 ③–⑥，它们都不挡分页 —— uncalibrated 的那几个常数影响的是
断行点的精度（L2/L4 断言）。所以顺序上有两条并行的路：**要么开分页**（纵向，Phase 3），
**要么补挤压上限那份 spike**（横向，把 L2 从 11/18 往上推）。两者不互相阻塞。
