...
- ~~图片：inline 为主，浮动只做不参与文字流的那种，其余环绕类型退化为 inline~~ ✅（2026-08-22）
  ...
  - `@uw/layout`：内嵌图占宽占高（`ObjectItem` 进断行与行高，底边坐在基线上）；
    **`wrap="none"` 的浮动图在文字流里占 0 宽**（印章、水印、衬在文字下的红头，
    一个字都不许被它挤走），位置等整页排完再按 `wp:anchor` 的参照物换算成**纸坐标**
    （`page.ts` 的 `placeFloats`）。其余环绕方式退化成内嵌
  ...
  - **没有真值**：图的底边到底是不是严丝合缝坐在基线上、六种 `relativeFrom` 各对应哪个框，
    两条都关在 `uncalibrated.ts`（`OBJECT_SITS_ON_BASELINE` / `FLOAT_RELATIVE_FROM_CALIBRATED`），
    各自写了能钉死它的样本。端到端有一份**合成的**带图 docx 走完全链
...
12k. ~~**Phase 5 的图片**~~ ✅（2026-08-22）四层各做一段：解析（`parse-drawing.ts`）→
    收字节（`images.ts`，按引用、key 带部件前缀）→ 占位（内嵌占宽高、`wrap="none"`
...
13. **上 Windows 之后按这个顺序补**：~~① 基线穿刺（第 6 步，解锁行盒与分页）~~ ✅
    ...
    ⑦ **图片的两个样本**（Phase 5 做完之后新增）：一是「五号字 + 一张 20pt 高的内嵌图 +
    五号字」，量图的底边 y 与同行文字的基线差多少、再套一个 `w:position` 量一遍 ——
    钉死 `OBJECT_SITS_ON_BASELINE`（顺带回答「图片参不参与网格吸附」）；
    二是同一张 10pt 见方的图复制六份、`positionH/@relativeFrom` 各取一种偏移都写 0，
    看六个 x 各落在哪条线上（奇偶那两种要排到第 2 页）—— 钉死
    `FLOAT_RELATIVE_FROM_CALIBRATED`。⑦ 只改图自己的位置，一行文字都不会动，
    所以排在 ③ 之后；但它是**唯一**能让「像不像 Word」的肉眼验收过关的一条

第 6 步曾经优先于任何**布局**代码 —— 与第 4 步同理，没测准的东西不要拿来当地基。
它做完之后的卡口是分页，分页现在也做完了（第 12f 步），于是**没有卡口了**：
每一行都有页号与 y，横向与纵向都能与真值逐行比。

接着做什么，按投入产出排（**DOM 渲染器 v1 / 域求值 / 页眉页脚都已做完**，见 Phase 2 / 3 / 5）：
...
2. ~~**图片**（`w:drawing`）~~ ✅ 做完了（Phase 5）：解析 → 收字节 → 占位 → 画，四层都通了，
   浮动的印章 / 水印也不再挤走文字。**接下来该给它一份真值**：`OBJECT_SITS_ON_BASELINE`
   与六种 `relativeFrom` 现在都只有观感与规范做依据，一份「五号字 + 20pt 高的内嵌图」
   的样本就能把前者钉死（要 Windows + Word）
