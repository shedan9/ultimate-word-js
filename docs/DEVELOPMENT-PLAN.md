...
- ⏸ DOM 渲染器 v1：绝对定位 + SVG text 逐字 x —— **不再被挡着**，分页已经给出页与 y
...
- ~~分页规则：widow/orphan、keepNext、keepLines、pageBreakBefore~~ ✅ 同上
  - **keepNext 是「接缝」不是「整块」**：本段末行与下一段首行同页即可，整段照样能拆。
    实现成排本段时把接缝高度算进可用高（`joinHeight`），下一段只有一行且自己也 keepNext 时
    接缝往下串（标题 + 副标题 + 正文首行这种三连）
  - 孤行寡行的 2 行下限、段前间距在页首算不算，两条**没有真值**，进了 `uncalibrated.ts`
  - 页是**惰性开**的：文末的硬分页符不会凭空多出一张空页，空节也不留垃圾页
- ~~y 与页码~~ ✅ 每行的绝对基线 = 版心顶 + 行顶 + 行内基线，gongwen-01 的 18 行
  最大误差 **0.06pt**（L3 判据 0.5pt），断言在 `layout/src/fixture.test.ts`。
...
1. **DOM 渲染器 v1**（Phase 2 最后一个 ⏸）：`DocumentLayout` → 绝对定位的页 + SVG
   逐字 x。它是第一次能用眼睛看见引擎的时刻，也是「截图回归」这条辅助手段的前提
2. **多页语料**（第 9 步）：Phase 3 的 DoD 是「20 页真实公文页数与 Word 一致」，
   而现在语料库里最长的 gongwen-01 只有一页 —— **分页规则至今没有一条真值验证过**，
   验过的只有「y 怎么累加」。造样本要 Windows + Word
3. **孤行寡行与页首段前间距的标定**（`uncalibrated.ts` 的两条），
   与上一条同一批样本就能做：用页边距把版心高度一格格调窄，看断页点怎么跳
4. **域求值**（PAGE / NUMPAGES + 收敛循环）：结构还原早就做完，就等分页
5. 第 13 步 ③–⑥ 的宽度类标定，影响的是 L2/L4 的精度
