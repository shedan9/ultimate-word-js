...
- ~~**拆行**与 `w:cantSplit`~~ ✅（2026-08-25）`@uw/layout` 的 `table-split.ts`
  ...
  - 切出来的是**两份各自自洽的 `RowLayout`**，不是「一份 + 裁剪窗口」：后者要渲染层加
    ...
    在布局里切完，渲染层只多认两个标记（`PlacedRow.continued` / `splitAfter`，
    用来决定接缝上画不画横线）
  - 不切的两种：`w:cantSplit`，以及**表头行** —— 它每页都要重复一遍，半行表头没有意义
  - **它修掉了一个真会错位的洞**：一行高过整页版心时，原来只能硬塞（`count = 1`），
    内容直接溢出版心且后面每页跟着错。现在会一页一片地切下去
  - 几处**没有真值**的判断（只改画法，不改断行）写在 `table-split.ts` 的文件头：
    上边距只算头片 / 下边距只算尾片、头片按 `top` 摆、`w:trHeight` 的富余记在尾片、
    接缝上画不画横线（`@uw/render-dom` 的 `SPLIT_ROW_SEAM_BORDER`）
...
12p. ~~**表格的几何与条件格式**~~ ✅（2026-08-26）第 13 步 ⑤ 与「接着做什么」第 5 条的
    ...
    判据多半是「用的**字体**是不是东亚字体」，但一个数据点分不开三种说法，
    钉死办法写在 `packages/fonts/src/metrics.ts` 的文件头，正好与第 13 步 ① 那份样本合并造。
...
接着做什么，按投入产出排（**DOM 渲染器 v1 / 域求值 / 页眉页脚 / 表格拆行都已做完**，
见 Phase 2 / 3 / 4 / 5 —— Phase 3 与 Phase 4 的 ⏸ 现在一个不剩）：
...
5. **表格剩下的两组样本**（几何与隔行带已经做完，见第 12p 步）：**格线冲突**的三条规则
   （第 13 步 ⑥）与**拆行**的三问（接缝上画不画横线、单元格边距在两片上怎么分、
   `w:trHeight` 的富余落在哪一片）。前者要读 PDF 里的**线**而不是文字 ——
   真值管线现在只抽文字片段与图片落点，得像当初为图片加 `pages[].images[]` 那样
   再加一路 `pages[].lines[]`（同样从算子表里读）；后者一张「第二行塞十几段文字逼它跨页」
   的表就能全钉死，造样本的工具已经会造表了
