...
3. **标点挤压 / 溢出**
   ...
   - **全角标点旁边不加中西文自动间距**（实测间隙 0.05pt 以内）——
     标点自己带着空半边，再加就成了双份。同一条对**歧义字符**也成立
     （`spike-width-01` 的 Ea4：宋体画的 `§` 与它旁边的 `B` 实测差 0.14pt），
     所以判据是**码点是不是无条件东亚**而不是「分到了哪个桶」

4. **中西文自动间距 `w:autoSpaceDE` / `w:autoSpaceDN`**（**已标定**，2026-09-05）
   - CJK 与拉丁字母 / 数字之间自动插入 **1/4 em** 间隙，默认开启
   - ⚠️ 这里原来写的是 **1/8 em**，那是抄来的说法、**从来没有过真值**
     （gongwen-01 的中西文之间本来就打了空格，量不到这个数）。
     `spike-width-01` 实测 36pt 的 `中B中` 两侧缝隙各 9.03pt = 0.25 em ——
     差的这 4.5pt 是**每一个中西文边界**都差
   - 那 1/4 em 按**接缝前面那个字符**的字号算，不是东亚那一侧的：
     `中`(36) + `E`(12) + `中`(36) 实测 9.03 / 2.99pt，把字号反过来就得到 3.00 / 9.05pt
   - 两个开关分家（DE 管字母、DN 管数字），四段交叉验过
   - 这个不做，中英混排的行长就永远对不上

5. **首行缩进的字符单位 `w:firstLineChars`**
   - 值是 1/100 字符，「首行缩进 2 字符」= `firstLineChars="200"`，实际宽度 = 2 × 当前字号的全角宽，**不是** 固定 twips

6. **歧义字符与中性字符的分桶**（**已标定**，2026-09-05，`spike-width-01`）
   - `§ ° ± × ÷ · ① ※ ℃ Ⅰ` 这些 EastAsianWidth = A 的字符**跟着 `w:hint` 走**，
     与两侧邻居无关：同一段 `B§B°B±B`，hint=eastAsia 时 `§` 是宋体的 1 em、
     hint=default 时是 Times 的 0.5 em
   - 中性字符里**只有空格**随邻居（任一侧是东亚字就用东亚字体的 0.5 em），
     且**与 `w:hint` 无关** —— 原来附带的「要 hint=eastAsia」是猜的；
     `/` `-` 两种 hint 下都待在 ascii 桶里
   - `w:hint` **只能改 XML 写进去**：Word 的对象模型里没有这个属性，
     见 `apps/fidelity/src/patch-docx.ts` 的 `patchRunHints`
...
- ~~`@uw/fonts`：脚本分桶、fontkit 解析、度量包、注册表（三级降级）、替换表、
  ...
  - `bucketOf()` 的歧义字符集直接取 Unicode **EastAsianWidth = Ambiguous** ——
    ...
    **2026-09-05 实测验过**（`spike-width-01`）：跟着 `w:hint` 走、与两侧邻居无关
  - `FontRun` 按「同字体 **且** 同脚本」切段：字体名相同也不跨 latin/eastAsia 边界合并，
    否则中西文间距（**1/4 em**，实测）没有边界可加
...
- ~~**空格随邻居分桶**~~ ✅（2026-08-21）见 §2.1。与悬挂两条合起来把 L2 从 18 行对 8 行
  ...
  2026-09-05 由 `spike-width-01` 补齐了它的两处边界（见 §2.2 第 6 条）：
  这条规则**与 `w:hint` 无关**（原来附带的「要 hint=eastAsia」是猜的），
  且中性字符里**只有空格**这么走 —— `/` `-` 老老实实待在 ascii 桶里
- ~~**歧义字符的分桶与中西文自动间距**~~ ✅（2026-09-05）第三份宽度类样本
  `spike-width-01` + `spike:width`（144 种组合逐行跑，31/31 唯一满分）。
  四条结论见 §2.2 第 4 / 6 条，其中**中西文间距实测是 1/4 em 而不是本文档一直写的 1/8**。
  gongwen-01 的 L2 没有变（16/18）—— 那份文档的中西文之间本来就打了空格，
  这也正是这个错能藏这么久的原因
...
> 未标定的常数（上下标与小型大写的字号系数、内建禁则集）集中在
> `packages/layout/src/uncalibrated.ts`，每一条都写了「拿什么样本能钉死」。
> 它们影响的都是**宽度**，也就是断行点；行高与基线那两条已经分别用 13 / 30 个样本定死了，
> 标点那一族（相邻挤压 0.5 em + 接缝要有空白、悬挂的半宽、临时挤压的上限与兑换率）
> 也已全部实测，搬去了 `break-class.ts`；**分桶与中西文自动间距**那一族 2026-09-05
> 也实测完了（`spike-width-01`，间距是 **1/4 em** 不是原先记的 1/8），搬去了 `items.ts` 的 `WIDTH_RULES`。
...
- ~~命中测试、坐标 ↔ 模型位置双向映射~~ ✅（2026-08-30）**布局空间那一半**做完了：
  ...
  - 为它给 `LineFragment` 补了 `contentIndex` / `offset`（片段首字在文件里的位置），
    并让**片段不跨内容片**：有了这条保证，「片段里第 k 个码点」的位置就是首字位置加长度，
    不必给每个字形再存一份。没有它，`LineFragment` 只说得出「这几个字属于哪个 run」，
    说不出是**哪几个**字
  - 摊平那一段的几何与 `@uw/render-dom` 的 paint.ts **一字不差**（画在哪儿、点在哪儿说的是
    同一件事），但**不能反过来依赖渲染层** —— 命中测试不该逼着调用方先渲染一遍
  - 两处把文档写在实现之前的说法改对了（见架构 §4）：②→① **不是二分查找**
    （表格让同一个 y 上并排坐着好几行，行序与 y 序不一致，二分的前提不成立）；
...
### Phase 7 — 编辑态
- ✅ **模型层文字事务地基**（2026-09-13）：`@uw/model` 的 `createTextEditor()`。
  同步 `tx` 原子提交，命令失败整批回滚；段内 `insertText` / `deleteRange` 支持跨 run 与文字片段，
  保留 run id、片段下标、样式与链接；正文表格内同样可用。冻结快照按修改路径共享子树，
  undo/redo 默认保留 100 个单元，显式 `origin: 'input'` 的单次相邻插入按 1000ms 窗口合并，
  `breakHistory()` 供光标移动等输入边界打断合并。纯数据 `TextChangeSet` 给出受影响段落与
  顺序文字修改，`mapTextPosition` / `mapTextRange` 负责已有位置的平移。
  空 run 与空段落可输入；拆段 / 合段 / 同一容器跨段删除已实现，域和格式命令仍待实现。
  判据：事务回归（含真实 gongwen-01 编辑 → 重新级联 / 查找 → 撤销恢复），不改布局算法或真值容差。
- ✅ **段落结构与输入接线**（2026-09-13）：`splitParagraph` / `joinParagraph` / 跨段删除，
  空段落锚点与可逆片段迁移；门面 `tx` / undo / redo 自动全量级联与重排，预提交检查失败不改历史。
  `mode: 'edit'` 支持点击 / 拖选、方向键、Enter、删除、多行纯文本粘贴与 IME 单次提交。
  textarea 在重排时常驻；多视图同步；调试台增加「编辑」开关。浏览器回归 `/tests/editing.html`。
  系统拼音输入仍待人工验收，增量排版与高级选区操作尚未完成。
- ✅ **按词导航与选词**（2026-09-22）：Ctrl / Option + 左右键、Shift 扩选、双击选词。
  整段拼接后用 `Intl.Segmenter('zh')` 分词 / 字素，跨样式的词语与 emoji 保持完整；
  选区 anchor / focus 分开保存，反向扩选和变更映射后输入位置仍跟随移动端。
  5 项新增单测覆盖跨 run / 片段、中文词、组合音标 / ZWJ emoji、段落边界、撤销和组合期保护；
  编辑浏览器回归当时 27 项通过。完整选区历史与高级剪贴板仍待实现。
- ✅ **跨页上下导航**（2026-09-22）：上下键按实际布局行移动，跨过段间距、页边距与空白补页，
  不进入页眉页脚、重复表头或无源域结果；同高的表格行按横向距离选择落点。
  连续移动记住纸坐标 x，短行、缩放和滚动不重置；左右移动、点击、显式选区或模型修改重新起算。
  Shift 保留 anchor，跨页滚动跟随 focus，组合期不接管方向键。
  6 项单测覆盖行距、短行、分页、表格、空段、软换行与跨样式字素，编辑浏览器回归增至 38 项。
  沿用索引的软换行字缝归下一行约定，尚无可在上一行行尾停靠的视觉 affinity。
- ✅ 常驻输入框 + IME 组合提交（§2.6），系统候选窗人工验收待做
- ✅ **按词删除**（2026-09-23）：Ctrl / Option + Backspace / Delete 与浏览器按词删除输入事件
  共用导航边界，跨 run / 文字片段的中文词、组合音标与 emoji 保持完整；经过的空白一起删除。
  已有选区只删选中范围，段落边界只合段，单次删除独立撤销 / 重做；组合期间不删除。
  5 项新增单测覆盖双向删除、跨样式恢复、选区、段落 / 文档边界与不可编辑片段原子回滚；
  编辑浏览器回归增至 52 项，覆盖快捷键、beforeinput、IME 保护、焦点与多视图同步。
- ✅ **编辑态纯文本复制 / 剪切**（2026-09-23）：从级联模型按选区取文，跨样式 / 片段拼接，
  保留段落与空段换行；自动折行 / 分页不额外换行，跳过隐藏文字与域代码，使用当前域显示值。
  剪切通过删除事务拒绝不可编辑范围，写剪贴板失败原子回滚；成功后折叠到起点，独立撤销 / 重做。
  组合期留给原生输入框，预览视图不能覆盖编辑选区的剪贴板内容。
  10 项新增单测与 15 项浏览器断言覆盖范围提取、失败保护、段落接缝和多视图同步；编辑回归增至 67 项。
  当前只提供 text/plain；表格内段落以换行分隔，对象使用 U+FFFC 占位，富文本剪贴板仍待实现。
- ✅ **全文选区与文档边界导航**（2026-09-23）：Ctrl/Cmd+A 选中模型正文；Ctrl+Home/End
  与 Cmd+↑/↓ 移到正文首尾，Shift 保留锚点，光标与滚动跟随 focus，并重置上下导航横向记忆。
  按文档序跨节下钻表格，保留首末空段；不包含页眉页脚，也不依赖可见页或重复表头。
  导航打断连续输入合并，组合期交给原生输入框；全选替换单次撤销，跨块容器删除仍原子拒绝。
  5 项新增单测覆盖空文档、跨节嵌套表格、反向扩选、替换撤销、边界刷新与组合期保护。
  17 项新增浏览器断言覆盖快捷键、跨页滚动、复制 / 替换 / 撤销和横向记忆重置，编辑回归增至 84 项。
- ✅ **字符格式命令**（2026-09-23）：`setRunProps(range, patch)` 在端点拆 run（前段保留 id，
  带迁移记录供选区映射），跨段落 / 表格 / 分节，覆盖段落标记时同步 `markRunProps`；值不变不拆、不记撤销。
  `null` 回到样式值。编辑态 Ctrl/Cmd+B / I / U 按级联格式「全是才取消」，折叠光标暂存到下一次输入
  （含 IME），移动 / 删除 / Enter 丢弃；`runPropsOfRange` 供工具栏状态。
  6 项模型单测 + 3 项控制器单测，12 项新增浏览器断言，编辑回归增至 96 项。
  不合并相邻同格式 run（保持 id 与片段下标稳定）；段落属性、列表与富文本粘贴仍待实现。
- ✅ **段落格式命令与对齐快捷键**（2026-09-23）：`setParagraphProps(range, patch)` 作用于范围触及的
  全部段落（含单元格段落），不拆 run、不动位置；`indent` / `spacing` 按字段合并、`null` 删项，
  段落标记格式拒绝（归 `setRunProps`）。Ctrl/Cmd+L / E / R / J 按级联对齐「已是则回左对齐」；
  `paraPropsOfRange` 供工具栏状态。2 项模型单测 + 1 项控制器单测，编辑浏览器回归增至 103 项。
  api.md 原设想的平铺参数（`firstLineChars` 在顶层）改为按 `ParaProps` 分组。
- ✅ **列表层级编辑**（2026-09-23）：`numbering` 按字段合并，只写 level 沿用样式的 numId、`numId: 0` 取消编号。
...
- ✅ **富文本剪贴板**（2026-09-23）：`fragmentOfRange()` 给出级联后的带格式片段（`textOfRange` 由它拼出，两份剪贴板取舍一致），
  ...
  源对齐跟着源段落标记，末块保留原段落对齐；Word 的 `<o:p>` 空段、`mso-list:Ignore` 编号、源码里折在汉字间的换行都处理了。
  修掉一处组合命令的坑：插入继承左边刚粘进来的 run，前一块改过的格式要显式还原。
  4 项 HTML 单测（jsdom）+ 1 项片段单测 + 2 项控制器单测，编辑浏览器回归增至 140 项。
  未做：图片与 docx 片段粘贴、Word 的 class 样式表（`MsoNormal` 里定义的字号不读）。`<br>` 与制表符见下一条。
- ✅ **制表位与软换行**（2026-09-23）：事务 `insertInline(position, 'tab' | 'lineBreak')`；`deleteRange` 可删制表位 / 换行 /
  符号 / 连字符（换成空 text 保留槽位，对象与域仍拒绝）；落在这类片段上的位置能继续输入与拆段。布局给制表位前后、
  换行后（下一行行首）补光标锚点，按字导航把可删片段算一个字。非列表位置 Tab 插制表位、Shift+Tab 留给浏览器，
...
- ✅ **表格里的 Tab 跳格**（2026-09-23）：`adjacentCellRange(body, nodeId, direction)`（`@uw/model` 的 `table-nav.ts`）
  ...
  控制器 `moveCell()` 选中整格，单元格里 Tab / Shift+Tab 优先于列表升降级；首末格接管但不动 —— 不该退回去插制表位
  （末格 Tab 加一行，同日由下一条补上）。Word 在格里插制表位用 Ctrl+Tab，浏览器截走（切标签页），没接。
  3 项模型单测 + 1 项控制器单测，编辑浏览器回归增至 172 项（`tableDocx()` 现场造 2×2 表）。
- ✅ **插行 / 删行 + 末格 Tab 加一行**（2026-09-23）：事务 `insertRow(position, 'above' | 'below')` / `deleteRows(range)`
  （纯函数在 `@uw/model` 的 `table-edit.ts`）。新行照所在行抄结构与格式（`trPr` 连 `w:tblHeader` 一起抄、`tcPr`、模板格首段的
  段落格式），每格一个空段落，返回新行首格；**纵向合并按网格列对齐**（不按格下标，跨列格会让下标错位）：插在合并区中间写
  `continue`、区外写 `none`；删掉合并区首格那一行时下面的续格升成 `restart`。删光就删整张表，容器空了补空段落；行里有域拒绝。
  位置映射：被删行里的位置收拢到下一行首格（`PositionMove.collapse`，新加 —— 按偏移平移会指到不存在的字），
  撤销插行时新行里的光标收拢回插行处。控制器末格 Tab 加一行、光标进新行首格。
  回写：新行以原有行为模板（`w:tr` 属性 / `trPr` / 每格 `tcPr`，只重写 `w:vMerge`，按 `tcPr` schema 顺序插），`w14:paraId` 不抄；
  **模板按结构比出来**：「A 下方插」与「B 上方插」模型顺序相同，只看顺序会让表头行下方的新行抄错 `trPr`。删掉的行不写，删光的表整个去掉。
  8 项模型单测 + 2 项回写单测 + 控制器单测扩充，编辑浏览器回归增至 177 项（加行 → 打字 → 导出再加载三行 → 两次撤销）。
  顺带修掉一处：宿主事务删掉光标所在整块且没给位置迁移时，编辑视图的选区映射抛错、整条刷新链断掉 —— 现在退到正文开头。
  未做：合并 / 拆分单元格、拆表（单元格里的分页符仍拒绝），没有快捷键（Word 本身也没有默认的）。插列 / 删列见下一条。
- ✅ **插列 / 删列**（2026-09-23）：事务 `insertColumn(position, 'left' | 'right')` / `deleteColumns(range)`（`table-edit.ts`）。
  列是**网格列**不是格下标：每一行各自看那条网格边界 —— 有格跨过就撑宽（`gridSpan` + 1）、落在 `w:gridBefore` / `w:gridAfter`
  里就加跳过列数、正好在两格之间就插新格（照目标那一侧的邻格抄 `tcPr` / `vMerge` / 首段格式）。新列宽照目标列、**整表变宽**，
  不从别的列里匀（固定列宽的表 Word 也往右长，长过版心照长；按界面行为写，未对过真值）。删列取两端格网格区间的并集，
  整格删、跨列格缩窄，删空的行删掉（没有格的 `w:tr` Word 报损坏），失去首格的续格升成 `restart`。没有 `w:tblGrid` 的表拒绝插列。
  回写：`w:tblGrid` 整组重写（`w:tblGridChange` 留着），`w:tblW` / `w:tcW` 只改 `w:w` / `w:type`，`w:gridSpan` / `w:vMerge` /
  `w:gridBefore` / `w:gridAfter` 按字段改（按 schema 顺序插）；新格照同一行里属性一致的原有邻格抄 XML，邻格全是新的
  （先插列再删掉原有列）时退到这一行原文里的格。插行抄模板行时也改成按**格**找模板（原来按原文的格下标，插过列就错位）。
  5 项模型单测 + 3 项回写单测，编辑浏览器回归增至 182 项（插列 → 打字 → 版面右移 → 导出再加载三列 → 删列 → 撤销）。
  未做：`pct` 宽的表插列后列宽的换算（照原样留着，布局按网格排）。合并 / 拆分见下一条。
- ✅ **合并 / 拆分单元格**（2026-09-23）：事务 `mergeCells(range)` / `splitCell(position)`（`table-edit.ts`）。
  合并区域先**撑成矩形**：两端格的网格区间取并集，再反复被跨列格（横）与纵向合并区（竖）撑大，直到没有格只露一半 ——
  与 Word 选区的行为一致；区域里有 `w:gridBefore` / `w:gridAfter` 的空缺就拒绝。内容按行、行内从左到右接进首格，
  只有空段落的格不贡献段落；段落是**搬**过去的（id 不变），所以位置不用映射，只有被丢掉的空段落收拢。多行合并时首行
  `restart`、下面每行留跨同样列数的 `continue` 格（Word 就这么存，不删行 —— 别的列在这些行里还有格）。
  拆分只做**合并的逆操作**（跨列拆回一列一格、纵向合并区逐格解开），Word 对话框里拆成任意行列数要新增网格，没做。
...
- 🟡 **增量排版第一步：段落缓存**（2026-09-22）：`ParagraphLayoutCache` 已接入门面加载 / 事务 /
  ...
  六份真实文档的缓存 / 全量结果逐项一致，编辑 / 拆段 / 撤销 / 重做回归通过，原真值容差不变。
  级联、表格装配、分页和视图更新仍完整执行；行内局部重排、分页提前止步和局部重绘待做。
- **DoD**：能连续用拼音输入法打完一整段中文，光标位置与撤销行为符合直觉

### Phase 8 — 回写 docx
- 模型 → OOXML 序列化，**保留未识别的原始 XML**（round-trip 安全：~~解析时把不认识的元素原样挂在节点上，回写时吐回去~~ →
  实现是以原文为底打补丁，见下条与架构 §1.4）
- ✅ **补丁式回写 + `doc.toDocx()`**（2026-09-23，新包 `@uw/serialize`）：`serializeDocx(pkg, body)` 只重写改过的部件，
  其余逐字节照搬（`OpcPackage.save`）。正文**走原文的 XML 树**：重新解析时 `parseBody(…, sources)` 记下「节点 id → 原元素」，
  没变的节点吐回原元素；改过的段落 / run 按**字段组**补丁 `w:pPr` / `w:rPr`（只动变了的那一个子元素、只改认识的属性，
  新元素按 schema 顺序插入 —— 顺序错 Word 报损坏），run 内容重写时图片 / 符号 / 域界桩从原元素池找回；
  拆出来的新段 / 新 run 以来源为模板，于是模板里不认识的格式（边框、高亮）跟着走，但不抄 `w14:paraId`；
  合段搬来的带链接 run 重新包 `w:hyperlink`；分节符跟着「本节末段」走；新建列表追加进 `numbering.xml`
  （abstractNum 必须排在全部 num 之前），没有就新建并登记内容类型与关系。
  字体槽同时有显式名与主题引用时**只写显式名** —— Word 让主题引用压过显式名，我们的级联相反，写两个会让 Word 与屏幕不一致；
  颜色改了同理丢掉 `w:themeColor`。
  判据：24 份 fixture 不编辑时每个部件逐字节相同；「输入 + 加粗 + 拆段 + 插制表位 + 段落格式」之后导出再加载，
  正文结构与编辑结果相等；合段带链接、书签 / 修订 / 边框 / 高亮 / 脚注引用保留、块级内容控件、分节末段拆分、
  新建列表（有 / 无 numbering.xml）、空段落输入、含图 run 改格式共 59 项单测 + 门面 2 项。调试台加了「导出 docx」按钮。
  未做：Word 打开无修复提示**只能人工验**（本机没有 Word 自动化）；页眉页脚 / 表格结构 / 样式的回写（编辑也还不支持）；
  `docProps/app.xml` 统计不更新。
...
12t. ~~**分桶的宽度那一半**~~ ✅（2026-09-05）第 13 步 ①b 的另外一半，也是「接着做什么」
    第 3 条。新样本 `spike-width-01`（3 页 31 段）+ `spike:width`（`WidthRules` 的
    3×4×3×2×2 = 144 种组合逐行跑，31 / 31 唯一满分，第二名 30/31），
    加上 `layout/src/width-fixture.test.ts` 进 CI。

    **读数方式是这一步的关键**：真值的 `TruthItem.font` 直接说出 Word 用哪款字体画了
    这个字（字体一换，PDF 里就换一次 `Tf`、起一个新片段），于是「§ 进了哪个桶」是
    **读**出来的而不是从宽度反推的。为此歧义字符只挑 **Latin-1 段**的
    （`§ ¨ ° ± ´ · ¤ × ÷`）：`※ ℃ ① Ⅰ` 这些看着更典型的公文符号 Times New Roman 里
    有的根本没有（实测 U+2460 就没有），量出来的字体名会变成**字体回退**的结果而不是分桶的结果；
    而且度量包只采样 0x20–0x7E 与 0xA0–0xFF，别的码点我们这边一律退到 `defaultAdvance`，
    Word 答得出、我们算不准，比不了。

    结论四条，两条是**真错**：① 歧义字符跟着 `w:hint` 走、与邻居无关（原来就是这么写的，
    这次是把它验了）；② 空格随东亚邻居这条**与 hint 无关** —— 原来附带的
    「要 hint=eastAsia」是猜的，且 `/` `-` 这类中性字符**不随**（这条原来也对）；
    ③ **中西文自动间距是 1/4 em 不是 1/8** —— 那个 1/8 是本文档 §2.2 抄来的说法，
    从来没有过真值，每一个中西文边界都少 4.5pt（36pt 字）；
    ④ 那 1/4 em 按**接缝前面那个字符**的字号算，不是东亚那一侧的。
    顺带照出一条**代码里的**错：`isLatinLetter` 把 Latin-1 的 `×`(U+00D7) 与 `÷`(U+00F7)
    当成了字母（它们夹在字母区间中间），于是 `中×中` 每个接缝平白多出 1/4 em ——
    一个字符的差别，只有把整段的行末 x 与真值对上才看得见。

    造样本这一步撞见的是**第二类「Word 造不出来」**：与 `spike-table-03` 的相邻边框冲突
    （Word 的对象模型表达不了那个局面）不同，`w:hint` 是 Word **不让你选**的开关 ——
    对象模型里根本没有这个属性，它由编辑器按输入环境自己写，同一页四段一样的文字
    Word 写出来的 hint 都能不一样（`spike-script-01` 的前八个 run 就是）。
    于是补丁那一路加了 `patchRunHints`，与边框补丁并列。
    同一步给 `make-fixture.ps1` 加了两样：`autoSpaceDE` / `autoSpaceDN` 两个开关，
    以及 **`runs`**（盖在 `text` 上的一层，段内逐段改字体 / 字号）—— 后者是回答
    「按谁的 em 算」的唯一办法，也顺手解开了「同一行两款东亚字体**字号还不同**时
    怎么合成行盒」那个未决项的样本限制（一段只能有一个字号）。

13. **上 Windows 之后按这个顺序补**：~~① 基线穿刺（第 6 步，解锁行盒与分页）~~ ✅
    ~~② A/C 类字体的度量包抽取~~ ✅ A/B/C/D 共 17 款已入库，L2 断言随之上线
    ~~①b 歧义字符与「东亚字体里的纯 ASCII 行」那份合并样本~~ ✅ **两半都做完了**：
    行高那一半是第 12s 步（`spike-script-01`，看**字体**不看字符），
    宽度那一半是第 12t 步（`spike-width-01`，歧义字符看 **`w:hint`**、
    中性字符里只有空格随邻居且与 hint 无关）。当时估的「挑两款字体都有的歧义字」
    是对的，但**范围估宽了**：真正能用的只有 **Latin-1** 那一段，
    `※ ℃ ① Ⅰ` 这些 Times New Roman 有的没有（字体回退会盖住分桶），
    而且度量包只采样到 0xFF，之外的码点我们自己也算不准
    ...
    中西文间距与空格分桶那两条 2026-09-05 也做完了，见第 12t 步 —— 前者实测是
    **1/4 em 不是 1/8**，后者两处边界的答案是「与 hint 无关」与「不跟着走」）：
    · 上下标字号、禁则集边界
...
14. ~~**Phase 6 的地基：布局索引**~~ ✅（2026-08-30）`@uw/layout` 的 `layout-index.ts`
    （命中测试 / range → 矩形 / 光标 / 文档序比较）+ `@uw/model` 的 `position.ts`
    （`DocPosition` / `DocRange`）。**跨平台，不需要 Word**，见 Phase 6 的条目。
    做的过程中被逼着改了三处**已有**的数据与文档：`LineFragment` 补上源位置并且不再跨内容片；
...
20. **Phase 7：文字事务地基** ✅（2026-09-13），见 Phase 7 条目与 api.md §10.1。
    下一步补齐空段落位置与段落结构命令，再接入选区、门面重排和 IME。
...
3. ~~第 13 步 ①b / ③ 的**宽度类**标定~~ ✅ 2026-09-05 做完了（第 12t 步，`spike-width-01`）——
   分桶两条、中西文间距两条，其中间距那条是**真错**（1/4 em 不是 1/8）。
   当时说的「`TruthItem.font` 直接说出 Word 用哪款字体」完全成立，
