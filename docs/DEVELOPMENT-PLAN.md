...
- ~~DOM 渲染器 v1：绝对定位 + SVG text 逐字 x~~ ✅（2026-08-22）`@uw/render-dom`。
  ...
  - 未画：图片、run 级高亮（model 没解析）、可选文本层、增量更新
...
2. **图片**（`w:drawing`）：现在 model 只留了个占位的 `ObjectItem`，渲染层连框都不画。
   公文里的红头、印章、附件截图全是图片，缺了它「像不像 Word」在肉眼验收上直接不及格；
   而且嵌入式图片**参与行高**，缺了行盒也会错。它现在是最大的一个洞
