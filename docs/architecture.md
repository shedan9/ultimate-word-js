...
| `@uw/view` | 🟢 屏幕坐标转换（页面仿射矩阵）· `locate` / `rectsOf` / `caretRect` · DOM 挂载 / 更新 / 销毁 · 缩放保留选区与布局索引 · 视口 ±2 页绘制 · 全文原生文字层 / 查找 / 纯文本复制 · **编辑态复制 / 剪切 / 粘贴**（模型选区、段落换行、当前域显示值、原子删除与写入失败保护；text/html 按 Word 写法带字体分桶与格式，粘贴分保留源格式 / 合并格式）· **制表位、软换行与分页符**（Tab / Shift+Enter / Ctrl+Enter，可删、可导航）· **表格里 Tab 跳格**（选中整格，最内层表格、跳过续格）· **编辑导航与删除**（按字素 / 词移动和删除、跨页上下导航与横向位置记忆、全文选区与文档首尾导航、Ctrl/Cmd+B/I/U 格式切换与折叠光标暂存格式、Ctrl/Cmd+L/E/R/J 段落对齐、列表 Tab 升降级与空项 Enter / 段首退格去编号、新建 / 取消列表（Ctrl/Cmd+Shift+L）、Ctrl+M 缩进、Ctrl+T 悬挂缩进、Ctrl+0 段前间距与 Ctrl+1/2/5 行距、Ctrl+Alt+1/2/3 标题与 Ctrl+Shift+N 正文（缺的内建样式补定义）、段尾回车按 `w:next` 换样式、Shift 扩选、双击选词，跨 run 分词与反向选区 focus，见 §8）· **装饰与 overlay**（住在页壳上，滚动与 CSS 变换由浏览器继承，缩放 / 重排 / 壳尺寸变化才重算）· **`scrollTo`**（位置 / range 首行 / 页，走 `scrollIntoView`）· **打印**（2026-09-13，`print.ts`：`<body>` 直下另造一份 pt 尺寸的打印页，`@page { size }` 取纸张、边距 0，Ctrl+P 与 `print()` 都走它，见 §8.5）；⏸ `below-text` 装饰 |
...
| `@uw/serialize` | 🟡（2026-09-23）补丁式回写：`serializeDocx(pkg, body)` 只重写改过的部件；正文以原文为底，没变的节点逐字节吐回，改过的段落 / run 按字段组补丁属性（`w:pPr` / `w:rPr` 按 schema 顺序插新元素）、图片 / 符号 / 域界桩找回原元素，拆出来的新段 / 新 run 以来源为模板（带走模型不认识的格式）；分节符跟着「本节末段」走；新建列表追加进 `numbering.xml`（没有就新建并登记内容类型与关系）。门面 `doc.toDocx()`。表格的**行与列**可插可删、格可合并拆分（2026-09-23，新行照原有行、新格照同行原有邻格为模板写，模板按结构比出来；`w:tblGrid` / `w:tblW` / `w:tcW` / `w:gridSpan` 按字段改）。格的合并 / 拆分同日接入（搬进首格的段落 id 不变，原元素逐字节吐回）。套用内建样式补的定义追加进 `styles.xml`（同日，没有就新建并登记）。⏸ 页眉页脚的编辑与回写、已有样式的修改（编辑本身也还不支持） |
...
**全文选区与文档边界（2026-09-23）**：输入控制器每次从模型正文的段落文档序取首尾位置，
跨节下钻嵌套表格，保留首末空段。Ctrl/Cmd+A 全选；Ctrl+Home/End 与 Cmd+↑/↓ 跳到对应边界，
Shift 扩选保留 anchor，滚动跟随 focus；这些导航清除上下移动的横向记忆并打断连续输入合并。
原先按键落到常驻 textarea，只能选中空输入框或移动其内部光标，无法操作正文。
范围不依赖分页、可见页或重复表头，页眉页脚不在正文树内；组合期间保留原生行为。
全选不放宽模型删除限制：同容器全文文字可单次替换和撤销，跨单元格 / 分节仍原子拒绝。

**字符格式切换（2026-09-23）**：模型的 `setRunProps(range, patch)` 只在范围端点拆 run，
前半段保留原 id，拆出的 run 带结构迁移记录，已存选区 / 批注经 `mapTextRange` 仍指着同一段字；
**不合并**相邻同格式 run —— 合并会改掉后方 run 的 id 与片段下标，多出来的 run 对排版无影响。
范围可跨段落、表格与分节（格式不改结构，没有删除那样的容器限制）；越过段尾或落在空段落时同步改
段落标记（`markRunProps`），空段首次输入与编号文字跟着它走。输入控制器按**级联后**的格式判断
「全部都是才取消」（加粗可能来自样式），折叠光标在非空段落里不改模型，格式暂存到下一次输入
（含 IME 提交），移动 / 删除 / Enter 丢弃。原先 Ctrl+B 落到常驻 textarea 上什么也不做。
段落格式 `setParagraphProps` 同一套写法，但只改段落节点、不拆 run，所以不产生位置迁移，选区原样保留；
`indent` / `spacing` 按字段合并 —— 整组替换会让「只改首行缩进」顺手抹掉左缩进。对齐快捷键按级联
...
段首 Backspace 去编号时把级联后的左缩进写成直接格式、首行 / 悬挂写显式 0（删掉会退回样式的首行缩进），
文字留在原处，与 Word 一致；空项 Enter 与按钮取消则回到页边，所以只在这一条路径上写。

**段落样式（2026-09-23）**：与编号定义同一个问题 —— 中文版 Word 的 styles.xml 里通常**有**「标题 1」（id 是 `1`，
按 `w:name` = `heading 1` 认，不按 id），但合成文档与别的生成器写的常常没有，Word 点样式库时才把定义写进去。
新增的定义同样挂在可编辑的树上（`Body.styles`，`StyleDefinition` 纯数据、没有 Map），`resolveBody` 用
`extendStyleSheet()` 在加载时那张表上补一层（按定义数组的引用缓存）；`tx.addStyle(definition)` 不记变更，
没有段落引用它时不留下。事务**看不到样式表**，所以「文档里有没有这份、id 撞不撞」由控制器判断
（门面把段落样式的 id / 名字 / `w:next` / 是否默认交给它）。套用样式清掉**全部**段落直接格式（含编号），
字符直接格式按「覆盖过半的那一项才清」—— 两条都照 Word 界面行为写，未逐格对过。
段尾回车按本段样式的 `w:next` 换样式，新段落要**一点直接格式都不带**：原来拆段一律继承（新段落里是一个
抄了格式的空 run），控制器在事务里没法把这个空 run 的格式清掉（非空段落里折叠范围的 `setRunProps` 不改任何东西），
于是给 `splitParagraph` 加了 `{ inherit: false }` —— 后段没内容时拆出一个没有 run 的真正空段落。
回写把新定义追加到 `styles.xml` 末尾（没有就新建并登记）；同一次回写可能新建 numbering 与 styles 两个部件，
内容类型清单与关系表要在同一份上改，分别从原包读会后者覆盖前者。
...
**富文本剪贴板（2026-09-23）**：`textOfRange()` 改为由 `fragmentOfRange()` 拼出 —— 片段是
「段落（对齐）→ 文字 + **级联后**的字符格式」的纯数据，剪贴板另一头不认识我们的样式表，只能带最终值。
HTML 的生成与解析在 `@uw/view` 的 `clipboard.ts`（model 不碰 DOM / CSS），两个方向都**只读内联 `style` 的原文**：
CSSOM 会丢掉 `mso-fareast-font-family`，而「汉字仿宋、数字 Times」只靠它表达。复制照 Word 的写法出，
于是 Word 与我们共用一套解析。粘贴按来源分「保留源格式」（自己 / Word / WPS）与「合并格式」（网页，只带开关）。
落到模型是已有命令的组合（`insertText` → `setRunProps` → `splitParagraph` → `setParagraphProps`），一个撤销单元；
一处容易错：`insertText` 继承**左边**的 run，也就是前一块刚粘进来的那个 ——「粗体 + 普通」会粘成两段粗体，
所以前面改过、这一块没写的键要显式还原成光标处原来的直接格式（跨段同样，拆出的空段继承拆点的格式）。

**制表位与软换行（2026-09-23）**：事务的 `insertInline(position, 'tab' | 'lineBreak')` 把 `w:tab` / `w:br` 插进 run 的
内容片列表；`deleteRange` 能删这类片段，删掉的换成空 text 占住槽位（与文字删空同一个理由：后面的 contentIndex 不挪）。
对象与域仍拒绝。三层各补一处，缺一处都用不了：① 模型 —— 落在制表位上的位置先换成文字位置才能 `insertText` / 拆段；
② 布局 —— 制表位和换行**不出文字片段**，于是它们两侧没有光标可放，`paragraph.ts` 给它们补光标锚点
（制表位前后各一个，换行之后那个在**下一行行首**），空文字槽找邻居时也把这些锚点算上，否则制表位后的空槽会落回制表位前；
③ 视图 —— 按字导航原来把一切非文字片段当 U+FFFC 屏障，退格删不掉制表位，现在可删的片段按一个字参与（制表位 / 换行当空白）。
分页符（Ctrl+Enter，同日）走同一条路：`insertInline(…, 'pageBreak')` 之后命令层再拆段。**布局一行没改** ——
`w:br w:type="page"` 所在行本来就带 `breakAfter: 'page'`、分页在那一行截断，换行之后的光标锚点落到下一行行首
（也就是下一页）。先拆段是照 Word 2013 起的行为：分页符后跟段落标记，否则后半段与前半段共用段落格式、编号也不会重数。
剪贴板里分页符记成 U+000C（原来与软换行一样记 `\n`，富文本复制再粘回来只剩软换行）：纯文本仍写换行，HTML 写 Word 的
`page-break-before` 那个 `<br>`。Ctrl+T 要「首行不动」就得把字符单位缩进换成 twips —— 控制器不认识字号，
由门面给每段带一个 `charUnit`（`@uw/layout` 的 `indentCharUnit`），这是 view 向 layout 借的唯一一条排版规则。
