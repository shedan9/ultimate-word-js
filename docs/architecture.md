...
| `@uw/fonts` | 🟢 行高规则 + 基线位置（两次穿刺实测标定）· **东亚 / 拉丁规则按字体选**（`eastAsianFont()`，`spike-script-01` 标定）· 混排行的行盒合成（`composeLineBox()`，同一份样本）· 脚本分桶（含**中性字符随邻居**：空格挨着东亚字就用东亚字体）· 度量包（**17 款已抽取入库**）· 注册表（三级降级）· `TextMeasurer` + 两级缓存 |
