...
| `@uw/view` | 🟢 屏幕坐标转换（页面仿射矩阵）· `locate` / `rectsOf` / `caretRect` · DOM 挂载 / 更新 / 销毁 · 缩放保留选区与布局索引 · 视口 ±2 页绘制 · 全文原生文字层 / 查找 / 纯文本复制 · **编辑态纯文本复制 / 剪切**（模型选区、段落换行、当前域显示值、原子删除与写入失败保护）· **编辑导航与删除**（按字素 / 词移动和删除、跨页上下导航与横向位置记忆、Shift 扩选、双击选词，跨 run 分词与反向选区 focus，见 §8）· **装饰与 overlay**（住在页壳上，滚动与 CSS 变换由浏览器继承，缩放 / 重排 / 壳尺寸变化才重算）· **`scrollTo`**（位置 / range 首行 / 页，走 `scrollIntoView`）· **打印**（2026-09-13，`print.ts`：`<body>` 直下另造一份 pt 尺寸的打印页，`@page { size }` 取纸张、边距 0，Ctrl+P 与 `print()` 都走它，见 §8.5）；⏸ `below-text` 装饰 |
...
- ⚠️ 原来这里写着「②→① 只是一次二分查找」，**做出来才看清是错的**：表格让同一个 y 上
  ...
  实现是**按页分桶后线性扫，取离点最近的行**（一页几十行），再在行内取离点最近的字缝。
- ⚠️ `DocPosition` 也不是原先写的两个字段，是**三个**（run + 内容片段下标 + 片段内偏移）：
  一个 run 的内容是一列片段（`w:t` / `w:tab` / `w:drawing`…），片段没有 id，
  而「run 内的全局字符偏移」要把前面每个片段的长度加起来 —— 那是模型才有的数据，
  索引这一侧（Worker 化之后主线程手上只有 `DocumentLayout`）算不出来。
  定义在 `@uw/model` 的 `position.ts`，因为它说的是模型树里的一点
- **`LayoutIndex` 是消费侧现建的**（`buildLayoutIndex(doc)`），不是流水线的产物 ——
  带方法的对象过不了结构化克隆（原则 1.1），与 model 的 `StyleSheet` 同理。
  它只吃 `DocumentLayout` 这一份纯数据，所以主线程拿到什么就能建出什么
...
**已实现（2026-09-22）**：`paragraph-cache.ts` 的 `ParagraphLayoutCache` 在段落装配入口复用
未变化段落；正文、嵌套表格单元格和页眉页脚共用。门面每份文档持有一份，加载 / 事务 / undo/redo
均透传。默认 LRU 2048 项，缓存只存段落相对几何，命中时克隆输出，避免公开布局被修改后污染后续帧。
级联、表格几何、分页、域求值迭代与视图更新仍完整执行；以下三级脏标记是后续目标：
...
缓存 key 当前直接序列化完整段落（含节点 id 与级联属性）、可用宽度、网格、文档设置、默认字体、
布局规则和本段 run 对应的域值，不用有限长度哈希，避免碰撞。早期设计只列「内容 + 属性 + 宽度」
会漏掉网格、禁则和域求值；全局域表整体入键则会让页码变化无谓地清掉无关段落。
更换度量器或 `TextMeasurer.revision` 变化会清空缓存。`FontRegistry` 注册 / 替换时递增版本，
`createTextMeasurer()` 同时刷新字宽、行高和字体脚本判定缓存。无版本的自定义度量器跳过段落缓存。
缓存实例留在消费侧，布局输出保持纯数据。判据是缓存与无缓存布局逐项一致及度量调用减少，
原有 Word 真值闸门和容差保持不变。
...
**已实现的模型基础（2026-09-13）**：`@uw/model` 的 `createTextEditor()` 只消费和修改直接格式
`Body`，输出冻结快照与可克隆 `TextChangeSet`。事务控制器有方法，留在消费侧；树与修改记录
仍是纯数据。run 与片段不拆不合，删除只清空文字，因此既有节点 id、样式与片段下标保持不变。
...
空段落以段落 id + `{contentIndex: 0, offset: 0}` 定位；首次输入生成真实 run，撤销还原空树。
布局仅增加空位置与空片段字缝元数据，不添加字形、不修改几何。结构命令限制在同一块容器，
保留直接格式和未修改 run id，片段迁移随变更集传播。模型不引入 DOM。
...
变更映射保留方向，组合输入期间忽略选词与移动。完整选区历史仍待实现。

**按词删除（2026-09-23）**：输入控制器的 `delete` 接受字素 / 词粒度，折叠选区时复用
导航得到的目标边界，再交给一次模型 `deleteRange` 事务；已有选区不扩展到词边界。
Ctrl / Option + Backspace / Delete 与 `deleteWordBackward` / `deleteWordForward` 共用此路径，
组合期保持模型不变。段落边界沿用合段限制；遇到非文字片段或跨块容器时原子回滚。
删除不走连续输入合并，因此每次按词删除都能独立撤销，重排继续复用段落缓存。

**编辑态剪贴板（2026-09-23）**：`@uw/model/textOfRange()` 从级联模型提取选区，
隐藏文字与域代码不进入输出，计算域用当前显示值；跨样式拼接，段落与空段以换行分隔，
自动折行与分页不改变文本。对象为 U+FFFC，表格按文档序的段落换行，当前只支持纯文本。
门面通过 `EditingBinding.text` 提供最新模型与域值，输入框接管 `copy` / `cut` 原生事件，
已处理事件不会再被只读视图的复制监听覆盖。剪切先在事务草稿上删除，再同步写入剪贴板；
任一步抛错保留模型、历史与选区。预提交校验在回调之后运行，因此校验失败时剪贴板可能已有文本，
原文仍保留。成功后单次提交、重排并折叠选区；组合期间由 textarea 自行处理。

**上下导航（2026-09-22）**：`vertical-navigation.ts` 直接消费 `DomView.index`，按物理页序和行顶
寻找上 / 下一条正文行，同高的格内行按字缝横向距离选目标。原先拿当前行高推算屏幕命中点，
跨大段间距、页边距或不同字号行时会停住或跳行；现在跳过空白补页、页眉页脚、重复表头与无源域结果。
连续移动的目标 x 存纸坐标 twips，经过短行、滚动和缩放不漂移；其他选区操作与模型变更时清除。
行内先拼接片段再切字素，避免停入跨样式组合音标 / emoji。软换行共享字缝沿用索引归下一行的约定，
只接受反查后仍属于目标行的落点，防止向上键停回原行；上一行行尾的视觉 affinity 尚未实现。
Shift 扩选沿用 anchor / focus，滚动跟随 focus；导航不更改布局算法或真值容差。
