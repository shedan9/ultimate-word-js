...
- `t.mergeCells(range)` 把两端所在格撑成的矩形并成一格：先按网格列取并集，再被跨列格 / 纵向合并区**撑大**到没有格只露一半
  （与 Word 的选区一样）。内容按行、行内从左到右接进首格，只有空段落的格不贡献段落（全空留首格原来的段落）；段落是搬过去的，
  id 不变，落在里面的位置照旧有效。多行时首行是 `restart`，下面每行留一个跨同样列数的 `continue` 格。`dxa` 宽按网格重算。
