...
- `t.setParagraphProps(range, patch)` 修改范围触及的每个段落（含其间的单元格段落，折叠范围即所在段）
  的直接段落格式，不拆 run、不移动位置，返回原范围。`indent` / `spacing` 按字段合并，字段 `null`
...
跨对象、制表位、域或不同块容器的删除仍由模型拒绝，保留模型、选区与历史。
...
选区与光标不变，一次一个撤销单元。`paraPropsOfRange(resolvedBody, range)` 返回对应的级联段落格式。
textarea 常驻挂载容器，提交后更新页树与全部挂载视图，重排不会替换输入节点。
组合输入期间不修改模型，`compositionend` 作为一次独立事务提交；尾随输入事件去重，
取消 / 失焦丢弃未提交组合串。缩放与滚动会刷新输入位置。

当前复用段落布局缓存，级联、分页和视图更新仍完整执行；行内局部重排、缩进 / 行距快捷键与列表命令、富文本剪贴板、屏幕阅读器编辑、
