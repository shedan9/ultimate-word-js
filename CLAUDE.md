...
输入节点重排时常驻，预提交检查失败保留模型和历史。
**段落布局缓存已接入**（2026-09-22）：正文、表格单元格与页眉页脚按完整段落输入复用断行结果，
域只比较本段显示值；字体注册 / 替换会递增版本，同时清除度量与段落缓存。
门面加载、事务与 undo/redo 共用有界缓存，级联、分页和视图更新仍完整执行；
行内局部重排、分页提前止步与高级编辑仍待做，见 api.md §10.1–10.3。
**按词导航与选词已接入**（2026-09-22）：Ctrl / Option + 左右键按词移动，Shift 扩选，
双击选词；`text-navigation.ts` 拼接整段后用 `Intl.Segmenter('zh')` 分词 / 字素，
跨样式的中文词、组合音标与 emoji 不被 run 边界切开。选区分别保存 anchor / focus，
反向扩选、变更映射后的光标与滚动跟随 focus。
**跨页上下导航已接入**（2026-09-22）：`vertical-navigation.ts` 按实际布局行移动，
跳过页眉页脚、空白补页、重复表头与无源域结果；连续移动保留纸坐标 x，经过短行、缩放和滚动也不漂移。
Shift 扩选保留锚点，滚动跟随 focus；其他编辑或选区操作重置横向记忆。编辑浏览器回归当时 38 项。
**按词删除已接入**（2026-09-23）：Ctrl / Option + Backspace / Delete 与
`beforeinput` 的 `deleteWordBackward` / `deleteWordForward` 共用按词导航边界；
包含经过的空白，已有选区只删选中范围，段落边界只合段，每次删除独立撤销。
跨样式词语与字素完整处理，组合期间不删除；不可编辑片段仍由模型事务拒绝并原子回滚。
编辑浏览器回归当时 52 项。
**编辑态纯文本复制 / 剪切已接入**（2026-09-23）：`textOfRange()` 从级联模型读取选区，
保留段落与空段换行，不受自动折行 / 分页影响；跳过隐藏文字、域代码和软连字符，域复制当前显示值。
剪切沿用原子删除事务并独立撤销，写剪贴板失败保留原文；组合期交给输入框原生处理。
表格段落以换行分隔，对象使用 U+FFFC 占位，富文本剪贴板仍待做。编辑浏览器回归当时 67 项。
**全文选区与文档边界导航已接入**（2026-09-23）：Ctrl/Cmd+A 选中正文，Ctrl+Home/End
与 Cmd+↑/↓ 跳到模型正文首尾，Shift 扩选保留 anchor，滚动跟随 focus。
按模型文档序下钻表格并保留首末空段，不受分页、虚拟化与重复表头影响；组合期不接管。
全选后替换沿用原子事务与单次撤销，跨单元格 / 分节或不可编辑片段仍受原有删除限制。
编辑浏览器回归当时 84 项。
**字符格式命令已接入**（2026-09-23）：`@uw/model` 的 `setRunProps(range, patch)` 只在端点拆 run、
不合并相邻 run（保持 id 与片段下标），跨段落 / 表格 / 分节，越过段尾或空段落时同步段落标记；
`null` 回到样式值。编辑态 Ctrl/Cmd+B / I / U 按**级联后**格式「全是才取消」，
折叠光标暂存格式到下一次输入（含 IME），移动 / 删除 / Enter 丢弃。编辑浏览器回归当时 96 项。
**段落格式命令已接入**（2026-09-23）：`setParagraphProps(range, patch)` 改范围触及的全部段落、
不拆 run 不动位置，`indent` / `spacing` **按字段合并**（整组替换会抹掉没提到的缩进）；
...
种类看 `NumberLabel.format`（`w:numFmt`）。段首退格去编号时把级联后的左缩进写成直接格式、
首行 / 悬挂写**显式 0**（删掉会退回样式的首行两字），文字不动；空项 Enter 与按钮取消不写。项目符号用 Unicode ● ○ ■ —— Word 的 Wingdings 私用区我们画不出来。
**缩进 / 行距快捷键已接入**（2026-09-23）：Ctrl+M / Ctrl+Shift+M 推到下一个 / 退到上一个
默认制表位的**整数倍**（不是加一个步长），字符单位缩进按两字挪（未对过），全列表选区改层级；
Ctrl+1 / 2 / 5 设多倍行距。只认 Ctrl：Cmd+M / Cmd+数字到不了页面，Windows 的 Ctrl+数字同样被截走。
编辑浏览器回归当时 132 项（列表部分用 `tests/list-fixture.ts` 现场造的 docx）。
**富文本剪贴板已接入**（2026-09-23）：复制同时写 `text/html`（`fragmentOfRange()` 的级联结果，照 Word 的
`mso-*-font-family` 写法分桶），粘贴优先 HTML —— 自己 / Word / WPS **保留源格式**、网页**只带开关且只加不减**；
解析在 `@uw/view` 的 `clipboard.ts`，只读内联 `style` 原文（CSSOM 会丢 `mso-*`）。容易错的一处：
`insertText` 继承**左边**刚粘进来的 run，前一块改过、这一块没写的格式要显式还原。编辑浏览器回归当时 140 项。
**制表位与软换行已接入**（2026-09-23）：`tx.insertInline(pos, 'tab' | 'lineBreak')`，`deleteRange` 能删制表位 / 换行 /
