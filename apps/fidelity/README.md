...
pnpm spike:script          # 脚本穿刺：纯 ASCII 的行走东亚还是拉丁规则、混排行怎么合成行盒
pnpm spike:width           # 宽度穿刺：歧义字符与空格进哪个桶、中西文自动间距有多宽

pnpm preview                     # 全部 fixture → out/*.html，用眼睛看引擎画成什么样
pnpm preview gongwen-01 -- --truth --debug   # 叠真值基线（红虚线）+ 画版心与行盒
```

这些 spike 脚本是**标定工具**，不是单测：它们从真值反推系数、打出残差表，
...
| `spike:script` | `spike-script-01` | 行高走东亚规则还是拉丁规则，看的是**实际画字的那款字体**（查它有没有 U+4E00 的字形），**逐段**判 —— 不是行里有没有东亚字符、不是 `w:eastAsia` 槽、也不是 `w:hint`；同一行里几款字体，**各自的行盒逐项取 max**（上取最高、下取最深），不是「取各自行高的最大值」 |
| `spike:width` | `spike-width-01` | 歧义字符（EastAsianWidth = A）**跟着 `w:hint` 走**、与两侧邻居无关；中性字符里**只有空格**随东亚邻居（任一侧就算），且**与 `w:hint` 无关**（`/` `-` 不随）；中西文自动间距是 **1/4 em 不是 1/8**，按**接缝前面**那个字符的字号算；靠 hint 才进东亚桶的歧义字符旁边**不加**间距（与全角标点同理） |
...
自动间距两个开关：`autoSpaceDE` / `autoSpaceDN`（`w:autoSpaceDE` / `w:autoSpaceDN`，
**默认开**，写 `false` 才关）。

**段内换字号 / 换字体**用 `runs`：它是**盖在 `text` 上的一层**，不另给文字，
每条只说「往后几个字符」和要覆盖的属性 —— `[{ chars, sizePt?, fontEA?, fontLatin?, bold? }]`。
字符数加起来对不上段落文字就抛（少算一个字会让后面每一条都盖到错的位置上，
而排出来的版看着毫无破绽）。在它进来之前一段只能有一个字号（`w:rPr` 挂在整段 range 上），
「中西文自动间距按**谁的** em 算」这一问根本摆不出来 —— 两侧字号相同，两种说法同解。
`@uw/fonts` 那个「同一行两款东亚字体**字号还不同**时怎么合成行盒」的未决项，
缺的也是这个能力。
...
## `w:hint` 也只能改 XML 写（`hint`）

与 `bordersRaw` 同一条路，但拦路的东西不一样：`bordersRaw` 是 **Word 表达不了**那个局面，
`w:hint` 是 **Word 不让你选** —— 对象模型里根本没有这个属性，它由编辑器按「这段文字是在
什么输入环境下进来的」自己写。证据就在已入库的样本里：`spike-script-01` 同一页四段文字、
同一套格式，Word 写出来的四个 `w:rFonts` 有的带 `w:hint="eastAsia"` 有的不带。

于是段落上多了一条 `hint`（`"default"` / `"eastAsia"` / `"cs"`），
`src/patch-docx.ts` 的 `patchRunHints` 在 Word 排完版之后把这一段里的每个 `w:rFonts`
都改掉，再交给 Word 导 PDF。定位同样按**整段的文字**且要求唯一 ——
所以 `spike-width-01` 里每一段都以 `Ea1、` 这样的标记开头。
那个标记末尾的全角顿号不是装饰：全角标点两侧不加中西文自动间距（已实测），
于是标记本身不会把「间距有多宽」的读数搅进「分桶」那几段里。
